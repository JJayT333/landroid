...
- 'deleteNode' — remove a branch and every descendant. Cascades into curative issues, map links, and lease-node references. BLAST-RADIUS RULE: if the target has any descendants, you MUST (a) call 'previewDeleteNode' first, (b) report the totals to the user in plain English ("this will remove X nodes, Y curative issues"), (c) wait for the user to confirm, and only then call 'deleteNode' with 'confirmCascade: true'. The tool refuses cascading deletes without that flag.
