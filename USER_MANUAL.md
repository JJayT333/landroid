...
- To lease an owner, hover a present mineral-owner card and click the `LEASE` action (next to `CONVEY`), or use `Add Lease` in the `Leasehold` view to pick an owner. Both open the Lease Purchase Report editor. A lease is an overlay on the present owner — it never changes mineral ownership — which is why `LEASE` is a peer of `CONVEY` rather than a conveyance. `LEASE` only appears on present mineral owners (not NPRI or fully-conveyed cards). The node edit modal also still exposes the same lease editor under its `Lease / Lessee Node` section.
- The Lease Purchase Report captures the instrument-level abstract (lessee, lease type, lease form — defaulting to Producers 88 (7-69) — dates, primary term, held-by-production, royalty, bonus/rental, comments) plus the per-tract slice (lessor interest, gross acres, and computed net mineral acres). Net mineral acres = gross acres x lessor interest and is the acre view of the same lessor interest used by the math; the descriptive abstract fields never change ownership, royalty, or NRI.
- When creating a lease from an unlinked title card, choose an existing owner in the editor if this is the same party already tracked in `Owners`. Leaving it blank creates a new owner record from the title card when you save.
