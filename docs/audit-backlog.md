...
| ACT-H03 | Design First | Decide whether the live ledger becomes durable now or is explicitly narrowed/reworded as in-memory shadow instrumentation. |
...
| ACT-H03 | Live title ledger is not durable despite durable wording | High | action-layer / title-cutover | current review; `src/store/title-action-log.ts`; `src/main.tsx` | Open | Action records and audit events are held only in a Zustand store. They are lost on refresh and not written to Dexie, `.landroid`, or a project-record bundle. | Either persist the live ledger behind an explicit version/storage gate or reword the branch as in-memory instrumentation only. | Pair | merge, future rebuild |
| ACT-H04 | Action log is not reset on workspace replacement | High | action-layer / title-cutover | current review; `src/store/workspace-store.ts`; `src/components/shared/Navbar.tsx`; `src/store/title-action-log.ts` | Open | Loading or importing a new project leaves the in-memory action records, audit events, and head hash from the previous project. That can mix ledgers across workspaces. | Reset or namespace the title action log on `loadWorkspace`, CSV import, `.landroid` import, demo load, and workspace key change. Add tests for two sequential workspaces. | Codex | merge, future rebuild |
...
| ACT-M03 | Default title command IDs are process-local | Medium | action-layer / audit-log | current review; `src/project-records/action-layer/title-command-sourcing.ts` | Open | The fallback command ID counter resets on reload/tab. Once persisted, repeated equivalent mutations can collide or become hard to trace. | Require caller-supplied operation IDs for live journaling or add a durable random/monotonic ID source. | Codex | future rebuild |
...
| ACT-L01 | `title-action-log.ts` header is stale about update coverage | Low | docs / code comments | current review; `src/store/title-action-log.ts`; `docs/phase-4-title-cutover-notes.md` | Open | The code now records `title.update`, but the header still says field edits are intentionally not recorded. Future reviewers can misread the scope. | Update the comment when the implementation is fixed or narrowed. | Codex | none |
...
| LLA-H04 | AI NPRI mutation tools default legally material unknowns | High | AI mutation / title math | `LINE_BY_LINE_AUDIT_2026-05-31.md` | Open | Direct AI tools still default unclear NPRI kind to fixed and fixed basis to burdened branch. A user can approve a legal assumption that was never sourced. | Require explicit `royaltyKind` for NPRI and explicit fixed basis when fixed; block approval previews until answered. | Codex | private beta, AI expansion |
| LLA-M01 | `.landroid` side-store rollback can leave mixed state | Medium | import/export / storage | `LINE_BY_LINE_AUDIT_2026-05-31.md` | Open | Parallel side-store replacement can keep mutating after rollback starts. Failed imports can mix old and new side-store data. | Pre-validate, then replace sequentially or wait all-settled before deterministic rollback. | Codex | private beta, future rebuild |
| LLA-M02 | Imported workspaces can keep stale node attachment badges | Medium | doc-registry / import | `LINE_BY_LINE_AUDIT_2026-05-31.md` | Open | Import hydration leaves node attachment summaries alone when Dexie has no rows. The UI can show document chips without backing documents. | Add strict import hydration mode that clears missing attachment summaries after workspace replacement. | Codex | private beta, future rebuild |
| LLA-M03 | Document attachment ordering crosses workspace boundaries | Medium | doc-registry / storage | `LINE_BY_LINE_AUDIT_2026-05-31.md` | Open | Reorder and position calculations still query by entity only. Matching node IDs in different workspaces can pollute ordering. | Scope attachment ordering and compaction by `workspaceId + entityKind + entityId`. | Codex | future rebuild |
...
| LLA-M08 | Hosted AI proxy exposes upstream/internal error details | Medium | hosted AI / security | `LINE_BY_LINE_AUDIT_2026-05-31.md`; `AI_SECURITY_STRUCTURAL_AUDIT_2026-05-20.md` | Open | Upstream error bodies can be returned to clients and logged as body prefixes. | Return generic client errors and log non-sensitive structured fields only. | Codex | hosted |
...
| LLA-M13 | CI push branch filters conflict with branch conventions | Medium | CI / repo process | `LINE_BY_LINE_AUDIT_2026-05-31.md` | Open | Approved `feat/*`, `fix/*`, and `docs/*` branches do not get push CI, while deprecated agent prefixes do. | Align push filters with current branch conventions or rely only on PR/main triggers and document that. | Codex | process reliability |
| LLA-M14 | Local root validation misses backend packages | Medium | CI / validation | `LINE_BY_LINE_AUDIT_2026-05-31.md` | Partial | CI validates backend packages now, but root local scripts still do not expose a single aggregate validation command. | Add `validate` or `validate:backend` scripts and update README/TESTING. | Codex | hosted |
| LLA-L01 | Local Ollama guidance recommends wildcard origins | Low | local AI / docs | `LINE_BY_LINE_AUDIT_2026-05-31.md` | Open | `OLLAMA_ORIGINS=*` broadens local Ollama browser exposure. | Recommend origin-specific local dev URLs instead. | Codex | none |
...
| LLA-L04 | `PATCH_PLAN.md` e2e status contradicts `TESTING.md` | Low | docs lifecycle | `LINE_BY_LINE_AUDIT_2026-05-31.md` | Open | The plan says skipped e2e remains, while testing docs say no Phase 5 skips remain. | Reconcile `PATCH_PLAN.md` or archive it if superseded by this backlog. | Codex | planning clarity |
| LLA-L05 | AI settings component comment is misleading | Low | docs / comments | `LINE_BY_LINE_AUDIT_2026-05-31.md` | Needs verification | Stale comment can make reviewers believe cloud keys persist. | Verify current comment and update if stale. | Codex | none |
| LLA-L06 | Stale fixture generator writes outside repo | Low | scripts / fixture control | `LINE_BY_LINE_AUDIT_2026-05-31.md` | Open | Running the documented generator can write artifacts outside the repo boundary. | Archive, delete, or require an explicit in-repo output path. | Codex | repo hygiene |
...
| DEF-ACT-04 | Define record-bearing `.landroid` v9 package format | High | package migration | `docs/phase-4-action-layer-notes.md`; `docs/project-record-migration-strategy.md` | Needs design | Action/audit records cannot be safely exported/imported until the package format and validation rules are explicit. | Design v9 dispatch, validation, rollback, round-trip, and future-version behavior before writing records into packages. | Pair | future rebuild |
