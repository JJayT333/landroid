...
| PER-008 | Import .landroid | Reject files with `version > LANDROID_FILE_VERSION (9)` | v10 file -> "Unsupported .landroid file version 10" | unit | yes | no | low | `workspace-persistence.test.ts` |
...
| PER-010 | Export .landroid | v9 export includes documentData + ownerData + mapData + researchData + curativeData; pdfData omitted; optional actionLedger appears only when ledger rows are present | Round-trip preserves all side stores and validates ledger rows when present | unit | yes — round-trip fixture | yes (side store scope) | low | unit |
...
The closeout performance capture also times a W2 `.landroid` UI export/import;
the 15.8 MB package was imported successfully, then removed from the working
tree while retaining checksum and size evidence.
